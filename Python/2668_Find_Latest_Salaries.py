...
def find_latest_salaries(salary: pd.DataFrame) -> pd.DataFrame:
    #@salary.sort_values(by = ["salary","firstname","lastname"], ascending = False, inplace = True)
    salary["new"] = salary.groupby(by = "emp_id").salary.transform("max")
    salary = salary[salary["salary"] == salary["new"]].sort_values(by = "emp_id").drop(columns = "new")
    return salary
...
SELECT
    ...
    department_id,
    RANK() OVER
        (PARTITION BY emp_id ORDER BY salary DESC) AS MaxSalary
FROM Salary) AS temp
WHERE MaxSalary = 1
