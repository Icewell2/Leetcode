...
def get_total_distance(users: pd.DataFrame, rides: pd.DataFrame) -> pd.DataFrame:
    rides = rides.groupby(['user_id'])['distance'].sum().reset_index(name = "traveled distance")
    temp = pd.merge(users, rides, how = 'left', on = ['user_id']).fillna(0)
    temp = temp.sort_values(by = ['user_id'], ascending = True)
    return temp
