...
#This will not work as it group by the original table of product_name and sale_date
# SELECT 
#     LCASE(product_name) product_name,
...
#     COUNT(sale_id) AS total
# FROM Sales
# GROUP BY LCASE(product_name),DATE_FORMAT(sale_date, '%Y-%m')
# ORDER BY LCASE(product_name),DATE_FORMAT(sale_date, '%Y-%m')
# ;
...
SELECT 
    LCASE(TRIM(product_name)) product_name,
    DATE_FORMAT(sale_date, '%Y-%m') sale_date,
    COUNT(sale_id) AS total
FROM Sales
GROUP BY 1,2
