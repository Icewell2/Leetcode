...
SELECT
    transaction_id
FROM Transactions
WHERE (DATE_FORMAT(day,"%Y-%m-%d"), amount) IN (SELECT days, max FROM cte)
ORDER BY 1 
;
...
# DENSE_RANK() OVER (PARTITION BY CAST(day AS date) ORDER BY amount DESC) AS ds
# FROM Transactions
# )
# SELECT transaction_id FROM a WHERE ds=1
