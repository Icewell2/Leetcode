...
SELECT
    m.member_id,
    m.name,
    IFNULL(COUNT(visit_id),0) AS visits
FROM Members m
LEFT JOIN Visits v
...
GROUP BY v.member_id
    ...
    SELECT DISTINCT
        ...
        m.member_id,
        m.name,
        IFNULL(100*COUNT(charged_amount),0) AS rate,
        visits
    FROM Members m
    LEFT JOIN Visits v
        ON m.member_id = v.member_id
...
SELECT
    member_id,
    name,
    (CASE WHEN rate/visits >= 80 THEN 'Diamond'
        WHEN rate/visits >= 50 AND rate/visits < 80 THEN 'Gold'
        WHEN rate/visits < 50 THEN 'Silver'
